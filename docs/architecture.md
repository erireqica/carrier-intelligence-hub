...
The operational data path is fully automatic when the standalone combined worker is running:
...
Gmail poller -> CarrierMessage(RECEIVED) -> message processor
    ...
    -> committed thread-label outbox -> Gmail label reconciler
        -> managed labels added/removed from the Gmail thread
```
...
Mailbox work runs in a standalone process, never in FastAPI startup. The thin `app.workers.pipeline` scheduler runs the existing poll, processing, and label units at independent cadences. This lets API availability, Gmail polling, AI work, and label delivery fail and recover independently. Each connection/message/thread uses short claim transactions and isolated sessions, so one failure does not terminate the pass.
...
The three trust boundaries are: sender eligibility controls ingestion; the model performs untrusted semantic interpretation; and deterministic backend validation controls materialization. Carrier identity always comes from the whitelist match, never from model output. `FOR UPDATE SKIP LOCKED` prevents two workers from owning the same message or label-sync generation. PostgreSQL commits business results before Gmail delivery, so a Gmail outage cannot roll back a Case or cause OpenAI to run again.
