...
`gmail.modify` is broader than Carrier Hub's feature. The application boundary deliberately exposes only message/metadata/attachment reads, label list/create, thread-label inspection, and `threads.modify` for the nine current app-managed user labels plus removal of the retired `Processed` label. It has no methods for sending, drafting, deleting, trashing, untrashing, archiving, forwarding, or changing read/unread/system-label state. The reconciler computes additions and removals exclusively from stored managed-label bindings; unrelated user and system labels are untouched.
...
The persisted `CarrierMessage` has `processing_status=RECEIVED`. Its `case_id`, `classification`, `summary`, and `priority` remain null because source ingestion is not semantic analysis. Later processing may create or update a Case even when the accepted `action_items` list is empty; it must not fabricate Tasks merely to justify the Case.

Durable duplicate protection uses the logical Gmail connection plus immutable Gmail message ID. Database uniqueness on `(gmail_connection_id, gmail_message_id)` prevents duplicate operational rows during repeated or racing polls, while `gmail_observed_messages` records the identity independently of `CarrierMessage`. That ledger survives intentional message deletion/reset and remains attached to the same logical connection through reconnect and verified same-mailbox handoff, so the same physical Gmail message cannot reappear as new work. Attachment uniqueness provides the corresponding per-message protection.
...
Carrier Hub does not claim Google production verification, a security assessment, HIPAA compliance, SOC 2 compliance, or production authorization approval. Gmail access uses a restricted Google scope. A production deployment must review and satisfy Google's then-current OAuth verification, user-data handling, and security requirements.
