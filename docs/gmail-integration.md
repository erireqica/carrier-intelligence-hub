...
Stage 3 connects a Carrier Hub user-owned Gmail inbox through Google OAuth 2.0 and ingests approved carrier mail as source records. Carrier Hub login proves who may operate the application; Google OAuth separately grants this application limited access to one Google account. The application never asks for or sees the Gmail password.
...
Unapproved messages are counted and discarded without fetching or storing their bodies. For approved messages, the parser walks MIME parts, prefers plain text, converts HTML to readable text as a fallback, decodes URL-safe base64, and tolerates malformed or missing optional fields. Attachment filename, MIME type, size, and stable Gmail attachment identity are stored; attachment bytes are not downloaded and text is not extracted.
...
Stage 3 also excludes Gmail labels, mailbox mutation, push notifications, attachment downloads, PDF extraction, AI classification/extraction, and automatic Case/Task/Review creation.
