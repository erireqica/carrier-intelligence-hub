...
def desired_labels_for_thread(
    ...
    if open_review is not None or any(
        message.processing_status is ProcessingStatus.NEEDS_REVIEW for message in messages
    ):
        desired.add(GmailLabelKey.NEEDS_REVIEW)
    if open_review is not None or open_task is not None:
        desired.add(GmailLabelKey.ACTION_REQUIRED)
    if any(
        message.processing_status is ProcessingStatus.FAILED
        and message.processing_next_retry_at is None
        for message in messages
    ):
        desired.add(GmailLabelKey.FAILED)
    if all(
        message.processing_status in {ProcessingStatus.PROCESSED, ProcessingStatus.IGNORED}
        for message in messages
    ):
        desired.add(GmailLabelKey.PROCESSED)
    latest = messages[-1]
    if latest.processing_status is ProcessingStatus.PROCESSED and latest.classification is not None:
        classification_label = CLASSIFICATION_LABELS.get(latest.classification)
...
def _listed_user_labels(response: Mapping) -> dict[str, str]:
    ...
    for item in response.get("labels", []) or []:
        ...
        if (
            ...
            and name in MANAGED_LABEL_NAMES.values()
        ):
